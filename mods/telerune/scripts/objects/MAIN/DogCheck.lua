...
function DogCheck:init(accurate)
...
function DogCheck:start()
    ...
    local cust_sprites_base = "dogcheck"

    if self.accurate then
        if Game.chapter == 1 then
            createDog("misc/dog_sleep", 0.8)
            playSong("dogcheck", 1, 1)
        ...
        local variant_choices = {"dance", "sleep", "maracas", "piano"}
        ...
        elseif self.variant == "piano" then
            ...
            local song_here = Utils.pick({"legend_piano", "snowdin_piano", "home_piano"})
            playSong(song_here)
...
function DogCheck:dogcheck2()
    local dog1 = Sprite("npcs/spr_dogcar", 0 - 40, 280)
    dog1.flip_x = true
    ...
    Game.world.timer:script(function(wait)
        wait(2.75)
        dog1:remove()
        local dog2 = Sprite("npcs/spr_dogcar", SCREEN_WIDTH, 280)
        ...
        local i = 0
        while (i < dognum) do
            local newdog = Sprite("npcs/spr_dogcar", 0 - 40, 280 + love.math.random(-80, 80))
            newdog.flip_x = true
        ...
        local i2 = 0
        while (i2 < dognum2) do
            local newdog = Sprite("npcs/spr_dogcar", SCREEN_WIDTH, 280 + love.math.random(-80, 80))
            newdog.physics.speed = love.math.random(-10, -16)
            newdog:setOrigin(0.5, 0.5)
